...
"3개의 점 좌표를 알면 하나의 2차 함수 곡률을 완벽히 그릴 수 있다"는 수학적 원리를 적용했습니다.
* **3회의 리더보드 프로빙(Probe):**
...
* **도메인적 의미:** 2024년 전체 데이터에서 앵커 모델이 특정 투수에게 틀렸던 편향(Bias)이 2025년에도 강력하게 지속되므로, 과거의 오차 방향을 **120% 강도로 신뢰**하여 밀어주는 것이 최적임을 의미합니다.
...
* **발견된 도메인 인사이트:** 시즌 후반기(7월 이후)에 나타난 투수들의 일시적인 구위 저하나 특정 코스 실투 급증은 다음 해까지 이어지는 고유 기량이 아니라, **"다음 해에는 원래의 정상 상태로 되돌아오는 단기 이상치(Mean Reversion)"**였습니다.
* **성공한 탐색 (음수 방향):** 따라서 후반기 과열 패턴을 반대로 깎아내는(수축) 음수 가중치 **$\beta = -0.3$**을 적용하자 리더보드 점수가 **$1072.56$**점으로 수직 상승했습니다.
